...
`autograd`은 딥러닝의 `Backpropagation`를 코드로 구현한 PyTorch의 하부 패키지입니다. 그래서 `Backpropagation`에 대해 먼저 숙지하고 본 글을 읽으시길 바랍니다.

| `Backpropagation`은 각 뉴런의 가중치<small>*weight*</small>를 업데이트하는 과정이다. <br> 그리고 그 업데이트 과정은 출력층에서 입력층 순서로 진행된다. |

<hr>

## `autograd`란?
`autograd`는 PyTorch에서 `Backpropagation`을 수행하는 PyTorch의 하부 패키지이다. `Backpropagation`을 수행하기 위해선 뉴런에서 행해지는 연산에 대한 Gradient<small>*미분값*</small>를 구해야 한다. `autograd`는 텐서 객체의 변수 `grad`에 Gradient 값을 담아준다.

**요약**: `autograd`는 텐서의 연산에 대해 자동으로 Gradient 값을 구해준다.

## 텐서 객체와 `autograd`
텐서를 생성할 때 `require_grad` 옵션을 `True`로 설정하면, 앞으로 그 텐서에 행해지는 모든 연산에 대한 Gradient를 계산해준다.

`require_grad` 옵션을 `True`로 하고 텐서 `x`를 만들어 보았다.
``` python
...
```
텐서 `y`는 `grad_fn`라는 멤버 변수도 가지고 있다. `grad_fn`은 해당 텐서 객체가 어떤 연산으로부터 생성되었는지를 알려준다.
...
이때, `out.grad`의 값은 d(out)/dx 값이다.

<hr>

결과를 신경망의 관점에서 해석해보자.

우리가 지금까지 연산(`*`, `+`, `mean()`)으로 텐서 `x`, `y`, `z`, `out`을 만든 과정은 **Loss function**<small>*Loss function*</small>을 만든 것이다.

처음에 만든 텐서 `x`는 **가중치**이다. 직접 만든 텐서 객체에는 `grad_fn` 값이 없다고 했는데, 가중치 자체는 어떤 연산으로부터 유도된 값이 아니기 때문에 `grad_fn`이 없는 것이 당연하다.

`out`은 Loss function의 값 자체를 의미한다. Backpropagation은 "각 뉴런층의 가중치를 갱신해주는 작업"이다. 그리고 그 과정에서 **Gradient** 값이 사용된다. Gradient 값은 텐서 객체의 `grad`에 담겨있다. 우리는 마지막에 `out.backward()`를 호출하고, `out.grad` 값을 확인하였다. `out.grad`는 d(out)/dx 값을 의미한다.
...
**GD**<small>*Gradient Descent*</small>에서는 d(Loss)/dw를 구하여 가중치를 갱신한다. 우리가 얻은 `out.grad`(=d(out)/dx)가 가중치를 갱신하는 d(Loss)/dw인 것이다.
...
이번엔 벡터 $\vec { y }$를 입력으로 갖는 스칼라 함수 $L = g\left( \vec { y } \right)$를 살펴보자. 함수 $L$의 Gradient인 벡터 $\vec { v }$는 다음과 같다.

<div>
$$\vec { v } = { \left( \cfrac { \partial L }{ \partial { x }_{ 1 } } , ..., \cfrac { \partial L }{ \partial { x }_{ n } }  \right)  }^{ T }$$
</div>

이제 벡터 $\vec { v }$와 Jacobian $J$를 곱할 것이다. 결과는 **Chain Rule**에 의해 다음과 같다. [^2]
...
가중치 $\vec { x }$는 어떤 연산들(=함수 $f$)을 거쳐서 벡터 $\vec { y }$는 만든다. 그리고 벡터 $\vec { y }$는 또 어떤 연산(=함수 $g$)를 거쳐서 Loss function $L$을 만든다.

벡터 $\vec { v }$는 Loss function $L$의 $\vec { y }$에 대한 Gradient인데, $\vec { v }$는 `grad_tensor`로 `backward()` 함수에 인자로 전달된다.[^3]

이제 [PyTorch Tutorial](https://pytorch.org/tutorials/beginner/blitz/autograd_tutorial.html#gradients)에 제시된 코드를 약간 변형하여 살펴보자.
...
non-scalar function인 `y`에 대한 기울기를 구하려면 vector-Jacobian Product를 사용해야 하고, VJP를 하려면 벡터 $\vec {v}$가 필요하다. 그래서 벡터 `v`를 생성한 후, `backward()` 함수의 인자로 전달하였다.
...
[^1]: vector-valued function $\vec { y } =f\left( \vec { x } \right)$은 함수를 vector 출력하는 함수이다. 이때, 입력이 벡터이기 때문에 함수 $\vec { y }$는 다변수 함수이다. 다변수 함수의 Gradient는 Jacobian $J$ 같은 행렬의 꼴로 표현된다.
...
[^4]: `x`를 `randn()`으로 생성했기 때문에 output 값에는 의미가 없다.
