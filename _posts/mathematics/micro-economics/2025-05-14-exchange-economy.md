...
그리고 이것을 종합해 어떤 교환 경제를 아래와 같이 표현 합니다.
...
"예산 집합"은 개인이 자신의 초기 보유량 $e(i)$를 모두 팔아서 얻는 소득으로, 구매 가능한 모든 재화 묶음의 집합 입니다.
...
"할당(Allocation)"은 배분 중에서도 충량 보존이 이뤄지는 배분 입니다. 처음 경계에 존재하던 재화의 총합을 넘지 않도록 배분하는 것을 말합니다.
...
만약, $(p, a)$가 경쟁 균형이라면, $(\lambda p, a)$도 경쟁 균형 입니다.

## Price Taker

모든 개인은 같은 가격에 직면 합니다. 그리고, 그 가격에 영향을 미치지 못 합니다.
모든 개인은 동일한 가격 체계 $p = (p_1, p_2)$를 기준으로 자신의 보시를 결정 합니다.
...
결국 두 식이 동일하기 때문에, 균형은 $\alpha = \beta$일 때만 성립 가능합니다!
...
결국 두 식이 동일하기 때문에, 균형은 $\alpha = \beta$일 때만 성립 가능하고, 이것은 가격 체계가 어떤 값을 가지든 성립합니다!
...
이것은 시장에서 모든 사람이 자신의 예산을 전부 다 쓰고, 총 공급량과 총 수요가 일치한다면, 재화 하나만 맞춰도 나머지 재화도 자동으로 제약 조건을 만족한다는 것을 의미합니다.
...
- 공급 조건을 만족해야 합니다.
  - 반대로 충분히 높은 가격 $p_i$에서는 모든 사람이 재화 $i$를 팔고 싶어 해야 합니다.

이 세 가지 조건이 모두 만족된다면, 어딘가에 균형 가격 $p^{\ast}$가 존재해서 수요와 공급이 일치하는 경쟁 균형이 성립합니다!

# Reopening Trade

"거래 재개"는 경쟁 규현에서 한번 거래가 끝난 뒤, 시장을 다시 열었을 때 무슨 일이 벌어지는지를 다룹니다.
