...
아, 미리 말해두지만 C언어와 `x86-gcc` 컴파일러를 기준으로 설명한다. 그리고 여기서 설명하는 `push`, `pop` 같은 명령어는 assembly 명령어를 의미하며, `x86-gcc`에서 사용하는 CISC 명령어셋을 따른다.
...
함수가 호출되고, 리턴 되는 과정에서 위의 레지스터들이 어떻게 동작하는지 살펴봅시다. 위의 그림과 함께 과정을 따라가봅시다
