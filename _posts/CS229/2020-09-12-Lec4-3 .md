...
이번 글에서는 앞에서 살펴본 Linear Regression, Logistic Regression 모델 들을 전부 포괄하는 일반화된 형태의 Linear Model인 GLM<sub>Generalized Linear Model</sub>을 살펴본다.
...
세상에는 셀수없이 많은<sub>countless</sub> Distribution이 존재할 것이다. 하지만 Gaussian Distribution이나 Bernoulli Distribution 같이 인간은 몇몇 Distribution을 수식의 형태로 정형화하고 분석하였다.
...
- $a(\eta)$는 distribution을 **normalize**하는 역할을 한다. $a(\eta)$을 잘 설정함으로써 distribution의 적분/덧셈값을 1로 만들 수 있다.

지금부터는 Bernoulli Distribution과 Gaussian Distribution이 Exponential Family에 속함을 살펴볼 것이다!
...
1. $y \vert x; \theta \sim \textrm{ExponentialFamily}(\eta)$<br>
  즉, 주어진 $x$, $\theta$에 대해 $y$의 확률 분포가 $\eta$를 파라미터로 하는 Exponential family의 확률분포를 따른다.
...
- Distribution의 parameter $\eta$는 Linear Model의 출력값이다.
- 우리의 입력 데이터와 목적에 따라 적절한 Distribution을 디자인 한다. = 적절한 $a$, $b$, $T$를 정한다. 
...
- 우리는 Distribution의 출력값을 통해서 Test를 진행한다.
  - Distribution의 값은 MLE에 따라 $\theta$ 값을 갱신하는 지표로 사용된다.

<hr>

### Ordinary Least Squares

지금까지 제시한 GLM의 형태를 Ordinary Least Sqares가 GLM의 특수한 경우임을 살펴봄으로써 곱씹어 보자.
...
위에서 나온 $\mu$와 같은 변수를 **canonical parameter**라고 부른다. canonical parameter는 Regression의 목적에 따라 설계한 Distribution이 같는 변수이다. 예를 들어 Bernolli Distribution에서는 $\phi$가 canonical parameter이다.
...
결국 sigmoid function은 그냥 나온 것이 아니라 Bernolli Distribution을 GLM으로 해석하여 유도된 결과인 것이다.
